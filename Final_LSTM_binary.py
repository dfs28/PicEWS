...
print(f'LSTM: Running with {length}h time target window, {input_length}h input window, using z-scores {no_z == False}')
...
for i in range(10):
    mortality_model = make_LSTM('death')
...
#LOS prediction
for i in range(10):
    discharge_model = make_LSTM('discharge')
...
for i in range(repeats):
    deterioration_model = make_LSTM('PEWS')
...
a_file = open("/mhome/damtp/q/dfs28/Project/PICU_project/files/LSTM_results_binary_pSOFA", "w")
...
results_df.to_csv('/mhome/damtp/q/dfs28/Project/PICU_project/files/LSTM_results_binary_' + file_suffix + '.csv')

tf.keras.utils.plot_model(full_model, to_file='/mhome/damtp/q/dfs28/Project/PICU_project/models/LSTM_binary.png', show_shapes=True, expand_nested = True)
tf.keras.utils.plot_model(deterioration_model, to_file='/mhome/damtp/q/dfs28/Project/PICU_project/models/LSTM_binary_single_output.png', show_shapes=True, expand_nested = True)